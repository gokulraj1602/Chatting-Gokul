var user;
$(function(){
  user = prompt("Please enter your name.", "Gokul");

  if(user != "Gokul" && user != "Vishnu"){
...
function showMessage(user, message) {
  if(user == userInfo[0]){
    ...
    $('<div class="message loading new"><img src=\'').appendTo($('.mCSB_container'));
    updateScrollbar();
    setTimeout(function() {
      $('.message.loading').remove();
      $('<div class="message new"><img src=\'').appendTo($('.mCSB_container')).addClass('new');
      setDate();
      updateScrollbar();
      i++;
...
function showMessage2(user, fileName, rawData) {
  if(user == userInfo[0]){
    ...
    $('<div class="message loading new">').appendTo($('.mCSB_container'));
    updateScrollbar();
    setTimeout(function() {
      $('.message.loading').remove();
      $('<div class="message new"> + fileName + '&nbsp;&nbsp;<img src="img/download.png" height="15px" width="15px" onclick="return saveFile(\''+fileName+'\')"/></div>').appendTo($('.mCSB_container')).addClass('new');
      setDate();
      updateScrollbar();
      i++;
      }, 1000 + (Math.random() * 20) * 100);
